...
"""Tests for slim.inception_resnet_v2. rewritten in tf contrib layers"""
...
class InceptionTest(tf.test.TestCase):
  ...
  def testBuildLogits(self):
    ...
    with self.test_session():
      ...
      logits, endpoints = inception.inception_resnet_v2_layers(inputs, num_classes)
  ...
  def testBuildNoClasses(self):
    ...
    with self.test_session():
      inputs = tf.random_uniform((batch_size, height, width, 3))
      net, endpoints = inception.inception_resnet_v2_layers(inputs, num_classes)
      self.assertTrue('AuxLogits' not in endpoints)
  ...
  def testBuildEndPoints(self):
    ...
    with self.test_session():
      inputs = tf.random_uniform((batch_size, height, width, 3))
      _, end_points = inception.inception_resnet_v2_layers(inputs, num_classes)
  ...
  def testBuildBaseNetwork(self):
    ...
    inputs = tf.random_uniform((batch_size, height, width, 3))
    net, end_points = inception.inception_resnet_v2_layers_base(inputs)
  ...
  def testBuildOnlyUptoFinalEndpoint(self):
    ...
    for index, endpoint in enumerate(endpoints):
      with tf.Graph().as_default():
        inputs = tf.random_uniform((batch_size, height, width, 3))
        out_tensor, end_points = inception.inception_resnet_v2_layers_base(
  ...
  def testBuildAndCheckAllEndPointsUptoPreAuxLogits(self):
    ...
    inputs = tf.random_uniform((batch_size, height, width, 3))
    _, end_points = inception.inception_resnet_v2_layers_base(
  ...
  def testVariablesSetDevice(self):
    ...
    with self.test_session():
      ...
      with tf.variable_scope('on_cpu'), tf.device('/cpu:0'):
        inception.inception_resnet_v2_layers(inputs, num_classes)
  ...
  def testGlobalPool(self):
    ...
    with self.test_session():
      inputs = tf.random_uniform((batch_size, height, width, 3))
      logits, end_points = inception.inception_resnet_v2_layers(inputs, num_classes)
      self.assertTrue(logits.op.name.startswith('InceptionResnetV2/Logits'))
      self.assertListEqual(logits.get_shape().as_list(),
                           [batch_size, num_classes])
      pre_pool = end_points['Conv2d_7b_1x1']
  ...
  def testGlobalPoolUnknownImageShape(self):
    ...
    with self.test_session() as sess:
      ...
      logits, end_points = inception.inception_resnet_v2_layers(
  ...
  def testUnknownBatchSize(self):
    ...
    with self.test_session() as sess:
      ...
      logits, _ = inception.inception_resnet_v2_layers(inputs, num_classes)
      self.assertTrue(logits.op.name.startswith('InceptionResnetV2/Logits'))
      self.assertListEqual(logits.get_shape().as_list(),
                           [None, num_classes])
      images = tf.random_uniform((batch_size, height, width, 3))
  ...
  def testEvaluation(self):
    ...
    with self.test_session() as sess:
      ...
      logits, _ = inception.inception_resnet_v2_layers(eval_inputs,
                                                num_classes,
                                                is_training=False)
  ...
  def testTrainEvalWithReuse(self):
    ...
    with self.test_session() as sess:
      ...
      inception.inception_resnet_v2_layers(train_inputs, num_classes)
